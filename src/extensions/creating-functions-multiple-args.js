...
function between(lower, upper) {
  const array = Array.from({ length: upper + 1 - lower }, (_, f) => f + lower)
  return array
}
...
function windClock(time, minutes) {
  const timeMins = parseInt(time.split(':')[1])
